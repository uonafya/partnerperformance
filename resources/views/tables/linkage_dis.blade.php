        ...
        <table id="{{ $div_id }}"  cellspacing="1" cellpadding="3" class="tablehead table table-striped table-bordered">
            <thead>
                ...
                <tr class="colhead">
                    ...
                    <th>Not Linked</th>                    
                    <th>Linkage</th>
                    <th><center>%Achievement(HTS_POS vs Monthly Target)</center></th>
                    <th>Contribution</th>
                    <th>
                </tr>
            </thead>
...
<script type="text/javascript" charset="utf-8">
    $(document).ready(function() {
        ...
        rows.forEach((row1,index)=>{
			row_total_array.push(parseInt(row1.pos))
		})
        ...
        rows.forEach((row,index) => {
            if(parseInt(row.pos) > 0){
                if(session == ''){
                    ...
                    var not_linked = parseInt(row.pos)-parseInt(row.tx_new) || 0 
                    // var gap = parseInt(target?.val / divisor || 0)-parseInt(row.pos) || 0 
                    var linkage = parseFloat((parseInt(row.tx_new)/parseInt(row.pos))*100)
                    linkage = linkage ? linkage.toFixed(1):0
                    var achieved = parseFloat((parseInt(row.pos)/parseInt(target?.val / divisor ))*100)
                    achieved = achieved >= 0 ? achieved.toFixed(1):0
                    var contribiutions = parseFloat((parseInt(row.pos)/parseInt(total_results))*100)
                    ...
                    row_html += '<td>' + row.tx_new + '</td>'
                    row_html += '<td>' + not_linked + '</td>'
                    row_html += '<td>' +linkage + '</td>'
