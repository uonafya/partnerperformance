            ...
            <thead>
                ...
                <tr class="colhead">
                    ...
                    @else
                     <th>County</th>
                    @endif
                    <th>TX New Results</th>
                    <th><center>% Achivement (TX New vs Target)</center></th>
                    <th>Contribution</th>
                </tr>
            </thead>
            <tbody id="vxv">
...
<script type="text/javascript" charset="utf-8">
    $(document).ready(function() {
        ...
        rows.forEach((row1,index)=>{
			row_total_array.push(parseInt(row1.tx_new))
		})
        ...
        rows.forEach((row,index) => {
            if(parseInt(row.tx_new) > 0){
                if(session == ''){
                    ...
                    var gap = parseInt(target?.val / divisor || 0)-parseInt(row.tx_new) || 0 
                    var gap_final = gap < 0 ? 0 : gap
                    var achieved = parseFloat((parseInt(row.tx_new)/parseInt(target?.val / divisor ))*100)
                    achieved = achieved >= 0 ? achieved.toFixed(1):0
                    var contribiutions = parseFloat((parseInt(row.tx_new)/parseInt(total_results))*100)
