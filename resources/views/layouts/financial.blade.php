            ...
            <div class="col-md-7">
                    ...
                    <div class="col-md-4">
                        <select class="btn filters form-control" id="filter_agency">
                            ...
                            @foreach ($agencies as $agency)
                                <option value="{{ $agency->id }}" @if (session('filter_agency') == $agency->id) selected @endif>
                                    {{ $agency->name }} </option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <div class="col-md-6">
                            <select class="btn filters form-control" id="filter_age_category_id">
                                ...
                                @foreach ($age_categories as $key => $age_category)
                                    <option value="{{ $age_category->id }}"> {{ $age_category->age_category }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                ...
                @elseif(ends_with(url()->current(), ['dispensing', 'tx_curr', 'gbv']))
                    ...
                    @if (isset($modalities))
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <select class="btn filters form-control" id="filter_modality"
                                    placeholder='Select Modality'>
                                    <option disabled='true'>Select Modality</option>
                                    <option value='null' selected='true'>All Modalities</option>
				...
				<a href="javascript:void(0)" onclick="date_filter('financial_year', 2021, '{{ $date_url }}')" class="alert-link"> FY 21 </a>| --}}
                    ...
                    @foreach ($financial_years as $financial_year)
                        <a href="javascript:void(0)"
                            onclick="date_filter('financial_year', {{ $financial_year->financial_year }}, '{{ $date_url }}')"
                            class="alert-link"> FY {{ $financial_year->yr }} </a>|
                    @endforeach
                    <br />

                    <a href="javascript:void(0)" onclick="date_filter('quarter', 1, '{{ $date_url }}')"
                        class="alert-link"> Quarter 1 </a>|
                    <a href="javascript:void(0)" onclick="date_filter('quarter', 2, '{{ $date_url }}')"
                        class="alert-link"> Quarter 2 </a>|
                    <a href="javascript:void(0)" onclick="date_filter('quarter', 3, '{{ $date_url }}')"
                        class="alert-link"> Quarter 3 </a>|
                    <a href="javascript:void(0)" onclick="date_filter('quarter', 4, '{{ $date_url }}')"
                        class="alert-link"> Quarter 4 </a>|

                    <br />

                    <a href='javascript:void(0)' onclick='date_filter("monthly", 1, "{{ $date_url }}")'
                        class='alert-link'> Jan </a>|
                    <a href='javascript:void(0)' onclick='date_filter("monthly", 2, "{{ $date_url }}")'
                        class='alert-link'> Feb </a>|
                    <a href='javascript:void(0)' onclick='date_filter("monthly", 3, "{{ $date_url }}")'
                        class='alert-link'> Mar </a>|
                    <a href='javascript:void(0)' onclick='date_filter("monthly", 4, "{{ $date_url }}")'
                        class='alert-link'> Apr </a>|
                    <a href='javascript:void(0)' onclick='date_filter("monthly", 5, "{{ $date_url }}")'
                        class='alert-link'> May </a>|
                    <a href='javascript:void(0)' onclick='date_filter("monthly", 6, "{{ $date_url }}")'
                        class='alert-link'> Jun </a>|
                    <a href='javascript:void(0)' onclick='date_filter("monthly", 7, "{{ $date_url }}")'
                        class='alert-link'> Jul </a>|
                    <a href='javascript:void(0)' onclick='date_filter("monthly", 8, "{{ $date_url }}")'
                        class='alert-link'> Aug </a>|
                    <a href='javascript:void(0)' onclick='date_filter("monthly", 9, "{{ $date_url }}")'
                        class='alert-link'> Sep </a>|
                    <a href='javascript:void(0)' onclick='date_filter("monthly", 10, "{{ $date_url }}")'
                        class='alert-link'> Oct </a>|
                    <a href='javascript:void(0)' onclick='date_filter("monthly", 11, "{{ $date_url }}")'
                        class='alert-link'> Nov </a>|
                    <a href='javascript:void(0)' onclick='date_filter("monthly", 12, "{{ $date_url }}")'
                        class='alert-link'> Dec</a>
